...
Then follow the guided setup. By default it selects the Bash or Zsh session that launched setup (falling back to `$SHELL` when needed) and safe defaults, so the normal path asks only which provider to use when several are installed, followed by one confirmation:

```sh
humansh setup
```

Setup names the provider, shell, and startup file it will change, then runs one provider check after confirmation. `--yes` runs it non-interactively. A healthy direct setup rerun keeps the saved settings with no questions or provider check; rerunning the installer from the other supported shell adds that integration without removing the existing one. Use `humansh setup --advanced` to change models, privacy, timeout, shortcuts, provider executables, OpenRouter, or configure every compatible shell. See [docs/setup.md](docs/setup.md) for details.
