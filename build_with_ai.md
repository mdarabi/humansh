...
A complete, bounded synopsis of the form `tool run [OPTIONS] TARGET [COMMAND] [ARG...]` can establish that the operand tail includes a forwarded command. Require at least one explicit required operand before `COMMAND`, a repeated argument tail, no listed subcommands, and no conflicting synopsis or options after operands. Validate leading options and their values, then leave all remaining words as inspectable positionals with partial coverage; never check the forwarded command's flags against the outer command or pass that tail to a help probe. Column-delimited option definitions can advertise custom lowercase value types such as `--network network`; consume those values before establishing the operand boundary. Unknown leading options and missing leading option values must still veto automatic execution and translation.
