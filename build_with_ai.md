...
A complete, bounded synopsis of the form `tool run [OPTIONS] TARGET [COMMAND] [ARG...]` can establish that the operand tail includes a forwarded command. Require at least one explicit required operand before `COMMAND`, a repeated argument tail, no listed subcommands, and one synopsis with no options after operands. Preserve separate invocation forms instead of flattening sibling lines, repeated command heads, or usage sections into a synthetic synopsis. Validate leading options and their values, and require a static unquoted operand or an explicit `--` to establish the boundary; a quoted or dynamic word that stopped leading-option parsing must fail closed. Mark the remaining words as forwarded with partial coverage; never check their flags against the outer command or pass that tail to a help probe. Exclude forwarded flag spellings from English-tail inspection while preserving inspection of following words, whose option-value roles are unknown. Column-delimited option definitions can advertise custom lowercase value types such as `--network network`; consume those values before establishing the operand boundary. Unknown leading options and missing leading option values must still veto automatic execution and translation.
